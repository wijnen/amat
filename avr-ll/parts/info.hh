...
namespace Info {
	...
	// Build pin struct.
	static const uint8_t _AVR_INT_ID[] PROGMEM { INT0_PIN _AVR_INT1 _AVR_INT2 _AVR_INT3 _AVR_INT4 _AVR_INT5 _AVR_INT6 _AVR_INT7 };
	static inline uint8_t first_int_pin() {
		return 0x78;
	}
	static inline uint8_t get_int_pin_id(uint8_t pin) {
		return pgm_read_byte(&_AVR_INT_ID[pin - first_int_pin()]);
	}
	static inline uint8_t next_int_pin(uint8_t pin) {
		uint8_t first = first_int_pin();
		if (pin < first)
			return first;
		if ((pin & 0x7) + 1 > _AVR_MAX_INT_PIN)
			return 0xff;
		while (true) {
			pin += 1;
			if (!(_AVR_INT_MASK & (1 << (pin & 7))))
				continue;
			return pin;
		}
	}
